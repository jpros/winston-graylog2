...
class Graylog2 extends Transport {
  ...
  log(info, callback) {
    ...
    const objDuplicate = info;
    objDuplicate["custom_timestamp"] = objDuplicate.timestamp;
    objDuplicate["custom_level"] = objDuplicate.level;
    delete objDuplicate.timestamp;
    delete objDuplicate.level;
    const cleanedMessage = JSON.stringify(objDuplicate)
